...
Version: 0.2.12.2 | Tests: 1931 with sklearn / 1813 without sklearn (ml tests skip gracefully) | C++ acceleration: up to 7.9x
...
- **V2.13 Phase 1 — MLAlpha Core** (`ez/portfolio/ml_alpha.py`, plan `docs/superpowers/plans/2026-04-06-v213-ml-alpha.md`): 走完 F8 第 1 阶段 (14 tasks + **2 轮 code review** — 第 1 轮 C1/I1 + 第 2 轮 codex 发现的 H1/H2/H3/MH1/M1). 实现 walk-forward ML 因子框架作为 `CrossSectionalFactor` 子类.
  - **构造器校验**: `__init__` 先校验 `model_factory` / `feature_fn` / `target_fn` 都是 callable (否则立即 `TypeError` 带参数名), 然后 probe `model_factory()` 返回值通过 whitelist, probe 本身抛异常也包装成 `TypeError` (配置错立即失败).
  - **V1 安全分层**: 严格 7 类 sklearn whitelist (Ridge/Lasso/LinearRegression/ElasticNet/DecisionTreeRegressor/RandomForestRegressor/GradientBoostingRegressor), `n_jobs=1` 在运行时 via `getattr(instance, 'n_jobs', None)` 检查, 用 `type(instance)` 身份比对而非 `isinstance` (阻止用户子类通过继承 monkey-patch `fit()` 绕过沙箱). `_assert_supported_estimator` 也在每次 `_retrain()` 重新验证 `model_factory()` 返回值, 防止工厂在不同调用时返回不同的 estimator 类.
  - **反 lookahead**: 两层防御 — (1) `slice_universe_data` 上游严格 `<` 切片 (V2.12.2 既有); (2) `_build_training_panel()` **positional purge (trading days)** 从尾部去除 `purge_days + embargo_days` 行. ⚠️ purge/embargo 是交易日单位不是日历日 — 原始实现用 `timedelta(days=N)` 日历日会让 label 穿过周末指向预测窗口, reviewer round 1 发现这个 C1 bug 后改为 `iloc[:-purge_bars]`, 与 `shift(-k)` 的行单位匹配.
  - **状态隔离**: portfolio walk-forward 通过 `strategy_factory()` 每折返回新实例获得隔离 (NOT `copy.deepcopy`). 单票 WalkForwardValidator 走 `copy.deepcopy` 路径 — MLAlpha 作为 generic Python value 也 deepcopy-safe. `portfolio_walk_forward` 3 splits × (IS+OOS) = 6 factory calls 的集成测试断言每个实例 id 唯一 + 初始状态清零.
  - **end-to-end 验证覆盖** (reviewer round 2 MH1 要求精确表述): **Ridge + RandomForestRegressor + GradientBoostingRegressor 三个 estimator 都有完整 `run_portfolio_backtest` 集成测试**, Ridge + RF 还额外有 `portfolio_walk_forward` 3-splits 集成测试 (断言 factory 每折返回独立实例 + oos_sharpes finite). Lasso/LinearRegression/ElasticNet/DecisionTreeRegressor 4 个 estimator 依赖 sklearn API 一致性 + 单元层 whitelist acceptance 测试, 没有单独跑 portfolio 回测链路 (它们的回测行为可以类推, 但不是"每个 estimator 都 end-to-end 证明").
  - **Deepcopy round-trip** 独立验证 Ridge/RF/GBR 三个 estimator — 这个路径服务于单票 `WalkForwardValidator` 的 `copy.deepcopy` 隔离, 以及 MLAlpha 作为 generic Python value 的可拷贝保证.
  - **In-memory 持久化**: 模型存在 `self._current_model`, 无磁盘 I/O (符合 sandbox 禁 `pickle`). 同一实例内 `retrain_freq` 窗口内的 compute 调用走 cache, 不重训.
  - **容错 + 可诊断性** (reviewer round 2 H2/H3/M1): `feature_fn`/`target_fn`/`_predict`/`model.predict` 异常都被 catch 并发一次性 warning log (带 exception type + msg + symbol), 不再静默吞掉. `_retrain` 里 `model_factory()` 和 `model.fit()` 都被 wrap, 异常后保留上一次模型, backtest 继续. "全 symbol 失败" 也会发一次性诊断 warning, 区分 "no signal" 和 "all symbols failed at predict". 一次性 flag (`_feature_fn_exception_warned` / `_target_fn_exception_warned` / `_predict_feature_exception_warned` / `_predict_call_exception_warned` / `_empty_panel_warned` / `_empty_predict_warned`) 防止 log spam. 价格为 0 造成的 `inf` 特征在 fit 前 `np.isfinite` 过滤.
  ...
  - **测试覆盖**: **97 个新测试** (`tests/test_portfolio/test_ml_alpha.py` 83 + `test_ml_alpha_sklearn.py` 14 — 前者含 callable 校验 + whitelist enforcement + lazy retrain + purge+embargo + anti-lookahead + outlier regression + determinism + cache + feature_fn/target_fn/predict 异常日志 + model_factory 运行时异常 + 空 panel 诊断 + edge cases + template render+exec + package exports; 后者含 Ridge/RF/GBR deepcopy + 跨实例确定性 + **Ridge/RF/GBR 三组 end-to-end run_portfolio_backtest** + **Ridge/RF portfolio_walk_forward factory-freshness** + fresh-instance no-state-bleed).
  - **依赖**: `scikit-learn>=1.5` (新增 `[ml]` optional group, 与 numpy>=2.0 ABI 兼容). 1813 → 1910 tests (+97).
