...
**数据快照物理落地**：
- **Gate-passed / 部署候选**：原始数据快照**长期保留**（冷存储可接受），确保可重放
- 普通 run（未通过 Gate）：近期（90天）Parquet 保留，远期只保留 hash + 元数据
- 远期重放：从数据源重新拉取并验证 hash 一致
...
│                                                      │
│  当前实现: 第1-3层已有 (V2.4), 第4-5层未实现          │
│  多重检验: V2.7 FDR; Agent: V2.8 自主研究循环        │
└─────────────────────────────────────────────────────┘
...
| **策略审批** | PM | Deploy Gate 审批 + 生命周期操作（V3.0+ 计划，未实现） |
