...
- Resident QMT session ownership has now been split into `qmt_session_owner.py`: `QMTSessionManager`, callback bridge, and `XtQuantShadowClient` live there; `qmt_broker.py` remains the broker adapter / gate surface and continues to re-export the owner symbols for compatibility
