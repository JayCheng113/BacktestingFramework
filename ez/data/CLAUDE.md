...
- V2.16.2 round 4: `DuckDBStore.get_data_hash()` — 12-char SHA-256 prefix of sorted file->md5 map + date_range. Used by `portfolio /run` to embed `run_config._data_hash` for cross-run reproducibility diagnostics. Excludes metadata (build_timestamp/version/symbol counts) so byte-identical rebuilds preserve hash. Returns None when manifest absent. 8 regression tests (`tests/test_data/test_data_hash.py`).
- V2.18: Parquet data cache — `DuckDBStore` checks `data/cache/{market}_{period}.parquet` before DuckDB tables. Both `query_kline()` and `query_kline_batch()` have parquet-first path with C4 date-range guard (manifest end + 7 days). `scripts/build_data_cache.py` bulk downloads from Tushare (stocks by trade_date + fund_daily/fund_adj for ETFs + index_daily for indices with market="cn_stock"). Cross-validates daily returns vs AKShare (hard gate: >1pp ERROR aborts, no parquet written). Weekly/monthly derived from daily. Coverage completeness gate (all required symbols must be present).
