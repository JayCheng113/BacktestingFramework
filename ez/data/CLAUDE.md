...
- V2.18: Parquet data cache — `DuckDBStore` checks `data/cache/{market}_{period}.parquet` before DuckDB tables. Both `query_kline()` and `query_kline_batch()` have parquet-first path with C4 date-range guard (manifest end + 7 days). `scripts/build_data_cache.py` bulk downloads from Tushare (stocks by trade_date) + BaoStock (ETFs, 100K/day limit) + Tushare (indices, market="cn_stock"). Cross-validates daily returns vs AKShare (hard gate: >1pp ERROR aborts, no parquet written). Weekly/monthly derived from daily. BaoStock raw/qfq sanity check.
